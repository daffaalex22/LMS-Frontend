...
const DiscussionItems = ({ thread }) => {
    const { data: student, isPending: studentPending, error: studentError } = useFetch('http://localhost:8000/students/' + thread.userId)
            ...
            >
                <Box>
                    <Avatar
                        alt="Remy Sharp"
                        src={student?.avatar}
                        src={thread.userType==="student" ? student.avatar : teacher.avatar}
                        sx={classes.ava}
                    />
                    <Box
                        sx={{
                    ...
                    >
                        ...
                        >
                            {student?.name}
                            {thread.userType==="student" ? student.name : teacher.name}
                        </Typography>
                        <Typography
                            variant="h5"
                            component="div"
